...
describe('MoviesService', () => {
  ...
  describe('getMovies', () => {
    ...
    it('filters out titles without primaryImage', async () => {
      mockFetch({
        titles: [makeTitle('tt001'), makeTitle('tt002', { primaryImage: null })],
        nextPageToken: null,
      })
      const result = await service.getMovies({})
      expect(result.movies).toHaveLength(1)
    ...
    it('maps missing rating, year, and runtime to N/A', async () => {
      mockFetch({
        titles: [makeTitle('tt001', { rating: null, startYear: null, runtimeSeconds: null })],
        nextPageToken: null,
      })
      const result = await service.getMovies({})
    ...
    it('maps missing genres to N/A', async () => {
      mockFetch({ titles: [makeTitle('tt001', { genres: [] })], nextPageToken: null })
      const result = await service.getMovies({})
      expect(result.movies[0].Genre).toBe('N/A')
    })

    it('maps missing plot to N/A', async () => {
      mockFetch({ titles: [makeTitle('tt001', { plot: null })], nextPageToken: null })
      const result = await service.getMovies({})
